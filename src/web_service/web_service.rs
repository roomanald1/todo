...
pub async fn start_webservice(http_state: Arc<Mutex<State>>) -> (Sender<()>, JoinHandle<()>){
            ...
            HttpMethod::Delete => {
                app = app.route(command.http_path, delete({
                    ...
                    move |Path(params): Path<Vec<(String,String)>>| {
                        async move { items_handler(shared_state, command, params.iter().map(|(_, value)| { String::from(value)}).collect()).await }
                    }
                }));
            },
            HttpMethod::Put => {
